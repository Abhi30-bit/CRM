...
function Coviewenq() {
    ...
    const getenq = async () => {
        ...
        const res2 = await axios.get(`http://localhost:5000/api/adduser/${localStorage.getItem('Counselor')}`)
        if (res.data.msg == "sucess" && res2.data.msg=="success") {
            console.log(res.data.enq);
            var enquries = res.data.enq.filter((a)=>{
                return a.assignto?a.assignto._id==localStorage.getItem('Counselor'):a.center== res2.data.adduser.center;
            })
    ...
    const updateenq = async (e) => {
        ...
        const assigndata = {enqid:editId,assignto:uid,assignby:localStorage.getItem('Counselor'),assignbyModel:"adduser",remark:rem};
    ...
    const addfollowup = async(e)=>{
        e.preventDefault();
        const followupdata ={'enqid':selectedEnq._id, 'uid':localStorage.getItem('Counselor'),status,nextdate,remark,program}
                    ...
                    <div className="row">
                        <div className="col-lg-3 mt-5 ">
                            ...
                            <div className="btn-group mt-4" role="group" aria-label="Basic radio toggle button group">
                                ...
                                <label className="btn btn-outline-primary" htmlFor="btnradio2">Cards</label>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h1 className="modal-title fs-5" id="exampleModalLabel">Assign Enquriy</h1>
                        ...
                        <form onSubmit={updateenq}>
                            <div className="modal-body">
                                ...
                                <div className="mb-3">
                                    ...
                                    <label htmlFor="recipient-name" className="col-form-label text-muted">Assign/Transfer to</label>
                                ...
                                <div className="mb-3">
                                    ...
                                    <textarea value={rem} onChange={(e) => setRem(e.target.value)} className="form-control" id="message-text" placeholder="Eg: transferred to manager for closure"></textarea>
                                </div>

                            </div>
                            <div className="modal-footer d-flex justify-content-center">
                                {/* <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Close</button> */}
                                <button type="submit" className="btn btn-primary w-50" data-bs-dismiss="modal">Assign</button>
    ...
    {selectedEnq && (
        ...
        {/* ACTION BUTTONS */}
      {
        selectedEnq.assignto &&   <div className="d-flex gap-2 mb-3">
          ...
          <button type="button" disabled className="btn btn-outline-warning w-100" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@mdo" onClick={() =>{assignfun(selectedEnq)}} >Transfer</button>
        </div>
      }
    ...
    <form action="post" onSubmit={addfollowup}>
       ...
       {/* TIMELINE */}
        ...
        {/* {
            filterfollow.map((f)=>(
                <div>
                 <h6>Status</h6>   {f.status} <h6>Date</h6> {f.nextdate}
                    <br />
                  <h6 >Remark</h6>  {f.remark }
                  <hr />
                </div>
            ))
        } */}
