...
function Mviewenq() {
    ...
    const getenq = async () => {
        ...
        const res2 = await axios.get(`http://localhost:5000/api/adduser/${localStorage.getItem('Manager')}`)
        if (res.data.msg == "sucess" && res2.data.msg=="success") {
            console.log(res.data.enq);
            var enquries = res.data.enq.filter((a)=>{
                return a.assignto?a.assignto._id==localStorage.getItem('Manager'):a.center== res2.data.adduser.center;
            })
    ...
    const updateenq = async (e) => {
        e.preventDefault();
        const d = Date()
        const data = { 'assignto': uid, 'assignby': localStorage.getItem('Manager'), assigndate: d };
        const res = await axios.put(`http://localhost:5000/api/enq/${editId}`, data);
        const assigndata = {enqid:editId,assignto:uid,assignby:localStorage.getItem('Manager'),assignbyModel:"adduser",remark:rem};
    ...
    const addfollowup = async(e)=>{
        e.preventDefault();
        const followupdata ={'enqid':selectedEnq._id, 'uid':localStorage.getItem('Manager'),status,nextdate,remark,program}
